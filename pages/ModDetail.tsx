...
import {
  ...
  Send, MessageSquare, CheckCircle, Key
} from 'lucide-react';
import { getModById, incrementDownload } from '../services/data';
import { supabase } from '../lib/supabase';
import { ModItem } from '../types';
import { useAuth } from '../contexts/AuthContext';
import { useToast } from '../contexts/ToastContext'; // Pastikan ini ada
...
const ModDetail: React.FC = () => {
  ...
  // Akses dibuka jika: tidak premium, user VIP, atau token sudah valid
  const canDownload = !mod?.isPremium || isVIP || isTokenValid;
  ...
  const handleValidateToken = async () => {
    if (!vipTokenInput.trim()) {
      showToast('Masukkan token terlebih dahulu', 'error');
      return;
    }
    setValidatingToken(true);
    try {
      const res = await fetch('/api/validate-vip', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token: vipTokenInput })
      });
      const data = await res.json();

      if (data.valid) {
        setIsTokenValid(true);
        showToast('Token Valid! Akses dibuka.', 'success');
      } else {
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="bg-[#141414] border border-zinc-800/80 rounded-2xl overflow-hidden shadow-2xl">
              ...
              <div className="p-6">
                ...
                <div className="border-t border-zinc-800/60 pt-5 text-zinc-400 text-sm leading-relaxed whitespace-pre-wrap">{mod.description}</div>
              </div>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-[#141414] border border-zinc-800/80 p-5 rounded-2xl sticky top-20 shadow-xl">
              ...
              {canDownload ? (
                <>
                  {isTokenValid && (
                    <div className="mb-3 px-3 py-2 bg-green-900/30 border border-green-700/50 rounded-lg flex items-center justify-center gap-2 text-green-400 text-xs font-bold">
                      <CheckCircle size={14} /> TOKEN AKSES VALID
                    </div>
                  )}
                ...
                <div className="text-center bg-zinc-900/60 p-5 rounded-xl border border-yellow-900/30">
                  ...
                  <p className="text-zinc-600 text-xs mb-4 leading-relaxed">
                    {isLoggedIn ? 'Upgrade role VIP atau gunakan Token untuk download.' : 'Login Discord atau gunakan Token.'}
                  </p>

                  {!isLoggedIn && (
                    <button onClick={login}
                      className="w-full flex items-center justify-center gap-2 bg-[#5865F2] hover:bg-[#4752C4] text-white font-bold text-sm py-2.5 mb-4 rounded-lg transition-colors">
                  ...
                  <div className="border-t border-zinc-800/60 pt-4">
                    <p className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold mb-2">Punya Token Lisensi?</p>
                    <div className="flex gap-2">
                      <div className="relative flex-1">
                        <Key size={12} className="absolute left-2.5 top-2.5 text-zinc-500" />
                        <input
                          type="text"
                          placeholder="Paste token..."
                          value={vipTokenInput}
                          onChange={(e) => setVipTokenInput(e.target.value)}
                          className="w-full bg-zinc-950 border border-zinc-800 text-white pl-8 pr-2 py-2 rounded-lg text-xs focus:border-green-700 outline-none transition-colors placeholder:text-zinc-700"
                        />
                      </div>
                      <button
                        onClick={handleValidateToken}
                        disabled={validatingToken}
                        className="bg-zinc-800 hover:bg-zinc-700 text-white text-xs font-bold px-3 py-2 rounded-lg transition-colors border border-zinc-700 disabled:opacity-50"
                      >
                        {validatingToken ? '...' : 'Check'}
                      </button>
                    </div>
                  </div>
                </div>
