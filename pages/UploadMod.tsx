...
const UploadMod: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!title.trim()) { showToast('Judul mod tidak boleh kosong!', 'error'); return; }
    if (!description.trim()) { showToast('Deskripsi tidak boleh kosong!', 'error'); return; }

    const sessionId = localStorage.getItem(SESSION_KEY);
    if (!sessionId) { showToast('Session expired, silakan login ulang', 'error'); return; }

    setSubmitting(true);
    try {
      // ✅ FIX: endpoint /api/mod bukan /api/mods
      ...
      if (!res.ok) {
        showToast(data?.error || data?.message || `Upload gagal (${res.status})`, 'error');
        return;
      }

      setSuccess(true);
      setUploadedModId(data.mod?.id || '');
      showToast(data.message || 'Mod berhasil diupload! 🎉');

    } catch (err: any) {
      console.error('[UploadMod] handleSubmit error:', err);
      showToast(err.message || 'Terjadi error, coba lagi', 'error');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center px-4">
      <div className="text-center space-y-5 max-w-sm">
        ...
        <p className="text-zinc-500 text-sm">
          {!['admin','administrator','owner','founder','co-founder'].some(r =>
            user.guildRoles.map(x => x.toLowerCase()).includes(r)
          ) ? 'Admin akan mereview dan mengapprove modmu.' : 'Mod sudah langsung dipublish.'}
        </p>
        <div className="flex gap-3 justify-center">
          ...
          <button
            onClick={() => { setSuccess(false); setTitle(''); setDesc(''); setDlUrl(''); setImgUrl(''); setTags([]); setModFile(null); setImgFile(null); setImgPreview(''); }}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="col-span-2">
              ...
              <input required type="text" value={title} onChange={e => setTitle(e.target.value)}
                ...
                className="w-full bg-zinc-900 border border-zinc-800 text-white px-3 py-2.5 rounded-xl text-sm focus:border-green-700 outline-none transition-colors placeholder:text-zinc-700"/>
            </div>
            <div>
          ...
          <div>
            ...
            <textarea required rows={4} value={description} onChange={e => setDesc(e.target.value)}
              ...
              className="w-full bg-zinc-900 border border-zinc-800 text-white px-3 py-2.5 rounded-xl text-sm focus:border-green-700 outline-none transition-colors resize-none placeholder:text-zinc-700"/>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input value={customTag} onChange={e => setCustomTag(e.target.value)}
                ...
                className="flex-1 bg-zinc-900 border border-zinc-800 text-white px-3 py-2 rounded-xl text-xs outline-none placeholder:text-zinc-700"/>
              <button type="button" onClick={addCustomTag}
                className="bg-zinc-800 hover:bg-zinc-700 text-zinc-300 px-3 py-2 rounded-xl text-xs font-bold transition-colors">
                + Add
              </button>
            </div>
          </div>
          ...
          <div className="border-t border-zinc-800/60 pt-6 space-y-5">
            ...
            <div>
              <label className="text-[10px] text-zinc-500 font-bold uppercase tracking-wider block mb-2 flex items-center gap-1.5">
                ...
                <span className="text-zinc-700 font-normal normal-case">(opsional)</span>
              </label>
              ...
              {imgPreview ? (
                <div className="relative rounded-xl overflow-hidden h-36 bg-zinc-900 border border-zinc-800">
                  ...
                  <button type="button"
                    onClick={() => { setImgUrl(''); setImgFile(null); setImgPreview(''); }}
                  ...
                  {imgUploading && (
                    <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center gap-2">
                      ...
                      <p className="text-white text-xs">Mengupload gambar...</p>
                    </div>
                  )}
                  {imageUrl && !imgUploading && (
                    <div className="absolute bottom-2 right-2 bg-green-900/80 text-green-400 text-[10px] px-2 py-1 rounded-lg font-bold">
                      ✓ Uploaded
                    </div>
                  )}
                </div>
                ...
                <button type="button" onClick={() => imgRef.current?.click()} disabled={imgUploading}
                  className="w-full border-2 border-dashed border-zinc-800 hover:border-green-700/60 text-zinc-600 hover:text-green-400 py-8 rounded-xl text-sm transition-all flex flex-col items-center gap-2 bg-zinc-900/20 disabled:cursor-not-allowed disabled:opacity-50">
                  {imgUploading
                    ? <><Loader2 size={18} className="animate-spin"/> Mengupload...</>
                    : <><Image size={18}/> Klik untuk upload thumbnail</>}
                </button>
              )}
              <div className="mt-2">
                <input type="url" placeholder="Atau paste URL gambar langsung..."
                  value={imageUrl}
                  onChange={e => { setImgUrl(e.target.value); setImgPreview(e.target.value); }}
                  className="w-full bg-zinc-900/50 border border-zinc-800/60 text-zinc-400 px-3 py-2 rounded-xl text-xs outline-none placeholder:text-zinc-700 focus:border-zinc-600 transition-colors"/>
              </div>
            </div>
            ...
            <div>
              ...
              <button type="button" onClick={() => modRef.current?.click()} disabled={modUploading}
                className={`w-full border-2 border-dashed py-8 rounded-xl text-sm transition-all flex flex-col items-center gap-2 disabled:cursor-not-allowed disabled:opacity-50 ${
                  downloadUrl && !modUploading
                    ? 'border-green-700/60 bg-green-900/10 text-green-400'
                    : 'border-zinc-800 hover:border-green-700/60 text-zinc-600 hover:text-green-400 bg-zinc-900/20'
                }`}>
                {modUploading
                  ? <><Loader2 size={18} className="animate-spin"/> Mengupload file...</>
                  : downloadUrl
                  ? <><FileCode size={18}/> {modFile?.name || 'File tersimpan'} ✓</>
                  : <><Upload size={18}/> Upload File Mod (.lua, .cs, .zip, dll)</>}
              </button>
              <div className="mt-2">
                <input type="url" placeholder="Atau paste URL download langsung..."
                  value={downloadUrl} onChange={e => setDlUrl(e.target.value)}
                  className="w-full bg-zinc-900/50 border border-zinc-800/60 text-zinc-400 px-3 py-2 rounded-xl text-xs outline-none placeholder:text-zinc-700 focus:border-zinc-600 transition-colors"/>
              </div>
            </div>
            ...
            <div>
              ...
              <input type="url" placeholder="https://youtube.com/watch?v=..."
                ...
                className="w-full bg-zinc-900 border border-zinc-800 text-white px-3 py-2.5 rounded-xl text-xs outline-none placeholder:text-zinc-700 focus:border-zinc-700 transition-colors"/>
            </div>
          </div>
          ...
          <div className="pt-2">
            <button
              type="submit"
              disabled={submitting || modUploading || imgUploading || !downloadUrl}
              className="w-full bg-green-700 hover:bg-green-600 disabled:bg-zinc-800 disabled:text-zinc-600 disabled:cursor-not-allowed text-white py-4 rounded-2xl font-black text-sm uppercase tracking-widest flex items-center justify-center gap-2 transition-all shadow-lg shadow-green-900/20 hover:shadow-green-900/40"
            >
              {submitting
                ? <><Loader2 size={16} className="animate-spin"/> Mengupload Mod...</>
                : modUploading || imgUploading
                ? <><Loader2 size={16} className="animate-spin"/> Menunggu upload file...</>
                : !downloadUrl
            ...
            {!downloadUrl && (
              <p className="text-center text-zinc-700 text-xs mt-2">
                ⬆ Upload atau paste URL file mod untuk mengaktifkan tombol publish
              </p>
            )}
          </div>
        </form>
