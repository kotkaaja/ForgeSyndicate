// components/ModManage.tsx — NEW
// Halaman user: lihat & manage mod sendiri
// Mod pending: TERLIHAT dengan keterangan "Menunggu Review Admin"

import React, { useState, useEffect, useCallback } from 'react';
import {
  Clock, CheckCircle2, XCircle, Edit3, Trash2,
  RefreshCw, AlertTriangle, Plus, Eye, EyeOff,
} from 'lucide-react';
import ProductCard from './ProductCard';
import type { ModItem as Mod } from '../services/data';
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-lg shadow-2xl">
        ...
        <div className="px-5 py-4 space-y-3 max-h-[70vh] overflow-y-auto">
          {[
            ...
            { label: 'Versi',         key: 'version',     type: 'text' },
            { label: 'URL Download',  key: 'download_url',type: 'url'  },
          ...
          <div>
            ...
            >
              {['Moonloader', 'CLEO', 'ENB', 'Texture', 'Map', 'Vehicle', 'Weapon', 'Script', 'Other'].map(c => (
...
const ModManage: React.FC<ModManageProps> = ({ sessionId, discordId, isAdmin = false }) => {
  ...
  const fetchMods = useCallback(async () => {
    ...
    try {
      // Ambil dari endpoint yang menampilkan mod sendiri + pending
  ...
  const handleDelete = async (mod: Mod) => {
    if (!confirm(`Yakin hapus mod "${mod.title}"? Tindakan ini tidak bisa dibatalkan.`)) return;
  ...
  const handleApprove = async (mod: Mod, approveAs: 'verified' | 'unofficial') => {
    try {
      const res = await fetch('/api/admin?action=manage-mod', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({
          sessionId, modId: mod.id, action: 'approve',
          data: { approval_status: approveAs },
        }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-white">
            {isAdmin ? 'Semua Mod (Admin)' : 'Mod Saya'}
          </h2>
          <p className="text-xs text-gray-400 mt-0.5">
            {isAdmin
              ? 'Kelola semua mod termasuk yang menunggu review'
              : 'Mod yang kamu upload — termasuk yang menunggu review admin'}
          </p>
        </div>
        <button
          onClick={fetchMods}
          className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs bg-gray-800 text-gray-300 hover:bg-gray-700 transition-colors"
        >
      ...
      <div className="flex flex-wrap gap-2">
        {stats.map(stat => (
          <button
            key={stat.label}
            onClick={() => setFilterStatus(filterMap[stat.label])}
            className={`
              flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium border transition-all
              ${filterStatus === filterMap[stat.label]
          ...
          >
            ...
            {stat.count > 0 && (
              <span className={`${stat.color} font-bold`}>{stat.count}</span>
            )}
          </button>
        ))}
      </div>
        ...
        <div className="flex items-start gap-3 p-3 rounded-xl bg-amber-500/10 border border-amber-500/30">
          ...
          <div>
            <p className="text-sm font-medium text-amber-300">
              {isAdmin
                ? `${mods.filter(m => m.approval_status === 'pending').length} mod menunggu review`
                : 'Beberapa mod kamu sedang menunggu review'}
            </p>
            <p className="text-xs text-amber-400/70 mt-0.5">
              {isAdmin
                ? 'Approve atau tolak mod di bawah ini agar bisa dipublish ke publik.'
                : 'Mod ini hanya bisa kamu lihat — tidak muncul di halaman publik sampai disetujui admin.'}
            </p>
          </div>
        </div>
      )}
      ...
      {loading && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="rounded-xl bg-gray-800 animate-pulse aspect-[4/5]" />
          ))}
        </div>
      )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredMods.map(mod => (
            <div key={mod.id} className="relative group">
              ...
              <ProductCard
                mod={mod}
                showPendingBadge={true} // Selalu tampilkan pending overlay di sini
              />

              {/* Action buttons overlay */}
              <div className="
                absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black/90 to-transparent
              ...
              ">
                <div className="flex gap-2">
                  ...
                  {/* Admin: Approve/Reject pending mod */}
                  {isAdmin && mod.approval_status === 'pending' && (
                    <>
                      <button
                        onClick={() => handleApprove(mod, 'verified')}
                        ...
                        title="Approve sebagai Verified"
                      >
                        <CheckCircle2 size={12} /> V
                      </button>
                      <button
                        onClick={() => handleApprove(mod, 'unofficial')}
                        ...
                        title="Approve sebagai Unofficial"
                      >
                        <Eye size={12} /> U
                      </button>
                      <button
