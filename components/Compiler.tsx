...
// ── Save compile history ke Supabase ──────────────────────────────────────
const saveCompileHistory = async (
  fileName: string,
  rawScript: string,
  arch: '32' | '64',
  ...
  try {
    await supabase.from('compile_history').insert({
      file_name:   fileName,
    ...
    // Jangan crash UI kalau DB gagal
    console.warn('[compile_history] Failed to save:', err);
  }
};

const Compiler = () => {
  const [file, setFile]         = useState<File | null>(null);
  const [fileContent, setFileContent] = useState<string>(''); // ← simpan raw content
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (selectedFile.name.endsWith('.lua')) {
        ...
        // ← Baca isi file untuk disimpan ke DB
  ...
  const handleCompile = async () => {
    ...
    try {
      ...
      // ← Simpan ke compile_history SETELAH berhasil compile
      // Ambil discord_id dari localStorage session kalau ada
      let discordId: string | null = null;
      const sessionId = localStorage.getItem('ds_session_id');
      if (sessionId) {
        try {
          const sesRes = await fetch(`/api/auth/session?id=${sessionId}`);
          if (sesRes.ok) {
            const sesData = await sesRes.json();
            discordId = sesData.discord_id || null;
          }
        } catch {}
      }

      await saveCompileHistory(file.name, fileContent, arch, discordId);
        ...
        <div className="bg-gray-900/50 backdrop-blur-sm border border-gray-800 rounded-2xl p-8 shadow-xl">
          ...
          {status === 'success' && (
            <div className="mb-6 p-4 bg-green-500/10 border border-green-500/50 rounded-lg flex items-center gap-3 text-green-400 animate-in fade-in slide-in-from-top-2">
              ...
              <p>Berhasil compile! File .luac otomatis terdownload & tersimpan ke database.</p>
            </div>
          )}
